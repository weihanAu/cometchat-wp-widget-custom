...
import { liveStreamClickHandler } from "../../../util/LiveStreamJs";
import BigScreenIcon from "../../../resources/icon/bigscreen.svg";
			...
			<div css={messageTxtWrapperStyle(this.context)} className="message__txt__wrapper">
				...
				>
					...
					{isVimeoLink(parsedMessage.props?.href) && (
						<button
							onClick={() => {
								liveStreamClickHandler();
							}}
						>
							<img width="16px" src={BigScreenIcon} />
						</button>
					)}
				</p>
			</div>
...
//function is vimoe link
function isVimeoLink(url) {
	const vimeoRegex =
		/^(https?:\/\/)?(www\.)?vimeo\.com\/(?:video\/|channels\/|groups\/[^/]*\/videos\/|album\/\d+\/video\/)?(\d+)(\/[a-zA-Z0-9_-]+)?(\/(?:[?&].*)?)?$/;
