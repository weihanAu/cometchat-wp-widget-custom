...
export class DockedLauncher extends React.Component {
  ...
  updateUnreadCount = () => {
    if (Object.keys(this.props.messagelist).length === 0) return false;
  ...
  render() {
    ...
    if (this.state.unreadcount) {
      notificationIcon = (
        <div css={notificationIconWrapperStyle()}>
          <span css={notificationIconStyle(this.props)}>
    ...
    let openIcon = (
      <img src={chatBubble} css={openLauncherIconStyle(this.state)} />
    );
    if (
    ...
    let closeIcon = (
      <img src={chatClose} css={closeLauncherIconStyle(this.state)} />
    );
    if (
      validateWidgetSettings(
        this.props.settings,
    ...
    return (
      <div
        css={launcherWrapperStyle(this.props, keyframes)}
        onClick={this.toggleView}
        className="app__launcher"
      >
