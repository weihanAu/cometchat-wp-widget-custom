...
export class Embedded extends React.PureComponent {
	...
	getStyle = () => {
		let customCss = "";
		console.warn(this.context.UIKitSettings);
		if (this.context.UIKitSettings.customCSS) {
			...
			>
				...
				>
					<FrameProvider>
						...
						<div
							css={embedContentWrapperStyle(sidebar, this.props, keyframes)}
							className="messenger__wrapper"
						>
							{sidebar}
							<div
								css={embedMainStyle(sidebar, this.state, this.props)}
								className="embedded__main"
							>
								{messageScreen}
							</div>
						</div>
